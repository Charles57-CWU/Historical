...
def new_class_vertex_info(line_length, class_count, class_to_plot, count_per_class_array, class_dict, class_color_dict):
    ...
    for i in range(class_count):
        if class_to_plot[i]:
            k = j + count_per_class_array[i] * line_length
            temp_starts = np.arange(j, k, line_length)
            new_index_starts = np.concatenate((new_index_starts, temp_starts))
            j = k

            temp_counts = np.repeat(line_length, count_per_class_array[i])
            new_vertex_counts = np.concatenate((new_vertex_counts, temp_counts))

            new_sample_count += count_per_class_array[i]

            new_class_vertices = np.concatenate((new_class_vertices, class_dict[i]))
            new_class_colors = np.concatenate((new_class_colors, class_color_dict[i]))
...
def new_marker_vertex_info(marker_per_line, class_count, marker_to_plot, count_per_class_array, marker_dict, marker_color_dict):
    ...
    for i in range(class_count):
        if marker_to_plot[i]:
            ...
            temp_counts = np.repeat(marker_per_line, count_per_class_array[i])
            new_vertex_counts = np.concatenate((new_vertex_counts, temp_counts))

            new_marker_count += count_per_class_array[i]

            new_marker_vertices = np.concatenate((new_marker_vertices, marker_dict[i]))
            new_marker_colors = np.concatenate((new_marker_colors, marker_color_dict[i]))
...
class showHideClassInfo:
    def __init__(self, class_dict, class_color_dict, marker_dict, marker_color_dict, class_to_plot, marker_to_plot, class_count, feature_count, sample_count,
        ...
        if plot_type == 'SPCP' or plot_type == 'DICP':
            ...
            line_length = int(feature_count / 2) + 1
            marker_per_line = int(feature_count / 2)
